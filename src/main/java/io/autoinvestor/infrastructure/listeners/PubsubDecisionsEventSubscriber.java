        ...
        try {
            ...
            if ("ASSET_DECISION_TAKEN".equals(event.getType())) {
                if (!event.getPayload().containsKey("assetId")) {
                    log.warn(
                            "Event payload missing 'assetId' field, ignoring event msgId={}",
                            msgId);
                    consumer.nack();
                    return;
                }
                if (!event.getPayload().containsKey("decision")) {
                    log.warn(
                            "Event payload missing 'decision' field, ignoring event msgId={}",
                            msgId);
                    consumer.nack();
                    return;
                }
                if (!event.getPayload().containsKey("riskLevel")) {
                    log.warn(
                            "Event payload missing 'riskLevel' field, ignoring event msgId={}",
                            msgId);
                    consumer.nack();
                    return;
                }
